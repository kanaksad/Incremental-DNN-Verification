...
class TestMNISTIncremental(TestCase):
    def test_mnist_base_diffai(self):
        args = ps.ShareArgs(net=config.MNIST_DIFFAI_5_100, tuned_net=config.MNIST_DIFFAI_5_100_THIRD_BLOCK, domain=Domain.DEEPZ, dataset=Dataset.MNIST, eps=0.001, k = 3)
        ps.proof_share(args)
